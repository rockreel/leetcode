...
class Solution:
    def maxProfit(self, k: int, prices: List[int]) -> int:
        ...
        for i in range(1, min(k + 1, len(prices) // 2 + 2)):
            max_money_left_buy_stock_after_profit_prev = float('-inf')
            for j in range(2, len(prices)+1):
                max_money_left_buy_stock_after_profit = max(
                    dp[i-1][j-2] - prices[j-2],  # After profit before prices[j-2], buy at prices[j-2]
                    max_money_left_buy_stock_after_profit_prev
                )
                max_money_left_buy_stock_after_profit_prev = max_money_left_buy_stock_after_profit
                dp[i][j] = max(
                    max_money_left_buy_stock_after_profit + prices[j-1],  # Sell at prices[j-1]
